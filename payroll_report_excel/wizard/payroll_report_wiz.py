...
class payrollreportexcelwiz(models.TransientModel):
    ...
    def get_item_data(self):
        ...
        if self.from_date and self.date_end:
            ...
            worksheet.write(row, 1, 'Job Position', cell_text_format)
            worksheet.write(row, 2, 'Employee RegNumber', cell_text_format)
            ...
            payslip_ids=self.env['hr.payslip'].search([('date_from','=',self.from_date),('date_to','=',self.date_end),('state','=','verify')])
            if payslip_ids:

                for payslip in payslip_ids:
                    name = payslip.employee_id.name
                    id = payslip.employee_id.job_id.name
                    tag = payslip.employee_id.registration_number
